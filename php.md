...
ここではWebアプリケーション開発のPHPコーディング規約を示します。基本的にはCakePHPの[コーディング規約](http://book.cakephp.org/2.0/ja/contributing/cakephp-coding-conventions.html)に準拠するような形で作成されています。
...
基本的に英語を使用してください。ただし、あまりに難解な英単語、もしくは日本固有の名称を使わざるをえない時は、ローマ字表記にしてください。

#### 名詞・動詞の使い分け

- クラス・変数・フィールドは、名詞を使用してください
- 関数・メソッドは、動詞を使用してください
- booleanを返すようなメソッドは、isHoge()とis〜で始めてください
- booleanを持つような変数は、$isHogeとis〜で始めてください
- find と getの使い分けは、findの場合は探す処理が発生し、getの場合はすぐ返せるようなニュアンスです
...
一つのメソッドには、一つの役割しか持たせないようにしましょう。引数によって、内部の挙動を大きく変えるようなメソッドはメンテナンスがしにくくなります。

## データチェック

### 変数が定義済みかチェックする

変数の定義・初期化が済んでいるかどうかを、isset() などの関数を使い必ずチェックしてください。

### empty()の使用について

ある変数にデータが入っているかどうかをチェックするのに empty() は使用しないでください。empty() は次のような値も空とみなします。
...
変数が空かどうかを判断するには、単純な値の場合は比較演算子（ != ）もしくは strlen() を使用してください。また配列の場合は、is_array() と count() を併用してください。
